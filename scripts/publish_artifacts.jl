...
run(
    `gh release create $TAG --repo $REPO --title "visidata $VISIDATA_VERSION" --notes "Bundled visidata $VISIDATA_VERSION artifact."`,
)
run(
    `gh release upload $TAG $(["artifacts/tarball-$p/visidata-$p-x86_64.tar.gz" for p in ("linux","windows")]) --repo $REPO`,
)

println("==> Writing Artifacts.toml")
for (platform, os, extra) in
    [("linux", "linux", Dict(:libc => "glibc")), ("windows", "windows", Dict())]
